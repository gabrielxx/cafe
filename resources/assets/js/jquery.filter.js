        ...
        filter: function(options) {
          ...
          function filter_function(event) {
            ...
            if (event.type=='keypress') {
              ...
              setTimeout(function(){
                filter_function({type:'after_paste', input:input});
              }, 1);
              return true;
          ...
          if (options.live) {
            if (parseInt(jquery_version[0]) >= 1 && parseInt(jquery_version[1]) >= 7) {
              $(this).on(options.events, filter_function);
            } else {
              $(this).live(options.events, filter_function);
            }
          } else {
            return this.each(function() {
              var input = $(this);
              if (parseInt(jquery_version[0]) >= 1 && parseInt(jquery_version[1]) >= 7) {
                input.off(options.events).on(options.events, filter_function);
              } else {
                input.unbind(options.events).bind(options.events, filter_function);
              }
            });
          }
...
jQuery(document).ready(function() {
  ...
  jQuery('input[data-mask]').each(function(idx) {
    ...
    jQuery(this).filter({ regex: regex, live: true }); 
  });
});
